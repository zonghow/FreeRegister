...
`rps_limit` 是账号级限制，默认每个 key `40 RPS`；同一个 key 的取号、查码、释放号码、余额和国家列表等 HeroSMS API 请求共享这一个窗口。后台首页会按 key 展示当前 API RPS，后台“接码配置”也可以直接切换 `api_key_strategy`。API key、专用代理、轮询间隔和自动释放等高级字段可在网页的 `config.toml` 编辑器中修改。
